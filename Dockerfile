...
# Configuration
COPY ngx_ceibacool_module/config ngx_ceibacool_module/config
...
RUN export CONF_ARGS=$(nginx -V 2>&1 | sed -n -e 's/^.*arguments: //p') && \
    ...
    sh -c "./configure ${CONF_ARGS} --with-debug --with-compat --add-dynamic-module=../ngx_ceibacool_module"
...
RUN cp -r /usr/share/easy-rsa . && \
    ...
    ./easyrsa --req-c="TW" \
              ...
              --req-email="mail@soyccan.tw" \
              --req-ou="Ceiba Cool" \
              --req-cn="ceiba.ntu.edu.tw" \
              --subject-alt-name="DNS:ceiba.ntu.edu.tw, IP:140.112.243.11" \
              --days="825" \
              --batch \
